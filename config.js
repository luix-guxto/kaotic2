...
// Bomber, se desejar desativar o auto-abrir navegador leia a página inicial da Íris na 
//const { bomber } = require("bomber-api") e
...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		+'VERSION:3.0\n'
		+'\nFN:test'
		+'\nCHARSET=UTF-8:Criador'
		+'\nTEL;TYPE=CELL,VOICE:5575988737769'
		+'\nEND:VCARD'
		...
		switch(command) {
		   ...
		   case 'translate':
			...
			case 'resposta':
				...
				// case 'criador':
				// 	await kill.reply(from, `☀️ - ADM Deyvisson: https://wa.me/${config.Owner[0].replace('@c.us', '')}`, id)
				// 	await kill.sendVCard(from, `BEGIN:VCARD\nVERSION:3.0\nFN:Deyvisson\nCHARSET=UTF-8:Deyvisson\nTEL;TYPE=CELL,VOICE:557588737769\nEND:VCARD`)
				// 	await kill.reply(from, `☀️ - ADM Luix Guxto: https://wa.me/553199949012`,id)
				// 	await kill.sendVCard(from, `BEGIN:VCARD\nVERSION:3.0\nFN:Luiz Guxto\nCHARSET=UTF-8:Luiz Guxto\nTEL;TYPE=CELL,VOICE:553199409012\nEND:VCARD`)
		   ...
		   case 'clima':
			...
			case 'kick':;case 'k':;case  'ban':;case 'b':;case '1':
				if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
					if (!isBotGroupAdmins) return await kill.reply(from, mess.botademira(), id)
					if (quotedMsg) {
			...
			case 'steal':;case 'lula':;case 'dilma':; case 'temer':
				if (!isxp) return await kill.reply(from, mess.needxpon(), id)
			...
			case 'entrargp':
				if(isOwner){
					if(args.length == 0) return await kill.reply(from, "ta faltando o link idiota", id)
					await kill.joinGroupViaLink(args[0]);
					await kill.reply(from, "entrei nessa porra", id)
					}else{
						await kill.reply(from, "sai daeee porra, tu não é meu dono", id)
					}
				break ;
				case 'gore':
					if (isGroupMsg && !isNsfw) return await kill.reply(from, mess.gpadulto(), id)
					await kill.reply(from, mess.wait(), id)
					let arquivosGore = await fs.readdir('./lib/media/gore/') // depois se quizer mudar o path
					var numeroGore = Math.floor(math.random()*arquivosGore.length)
					if(numeroGore==arquivosGore.length){
						numeroGore--;
					}//se mudar o path mude no sendFile tb
					await kill.sendFile(from, './lib/media/gore/'+arquivosGore[numeroGore], 'gore'+arquivosGore[numeroGore], '', id)
					break

					case 'porno':
						if (isGroupMsg && !isNsfw) return await kill.reply(from, mess.gpadulto(), id)
						await kill.reply(from, mess.wait(), id)
						let arquivosPorno = await fs.readdir('./lib/media/porn/') // depois se quizer mudar o path
						var numeroPorno = Math.floor(math.random()*arquivosPorno.length)
						if(numeroPorno==arquivosPorno.length){
							numeroPorno--;
						}//se mudar o path mude no sendFile tb
						await kill.sendFile(from, './lib/media/porn/'+arquivosPorno[numeroPorno], 'porn'+arquivosPorno[numeroPorno], '', id)
						break

						case 'hentai':
							if (isGroupMsg && !isNsfw) return await kill.reply(from, mess.gpadulto(), id)
							await kill.reply(from, mess.wait(), id)
							let arquivosHentai = await fs.readdir('./lib/media/hentai/') // depois se quizer mudar o path
							var numeroHentai = Math.floor(math.random()*arquivosHentai.length)
							if(numeroHentai==arquivosHentai.length){
								numeroHentai--;
							}//se mudar o path mude no sendFile tb
							await kill.sendFile(from, './lib/media/hentai/'+arquivosHentai[numeroHentai], 'hentai'+arquivosHentai[numeroHentai], '', id)
							break
			...
			case 'ddd':
				...
				if(!isGroupMsg) return await kill.reply(from, `Por que está fazendo sorteio no privado?`, id)
				   ...
				   var experiencia = args[0]
				   if(experiencia == null || experiencia == NaN || experiencia == undefined || experiencia == 0) return await kill.sendTextWithMentions(from, `Qual o valor do premio?\ntem que colocar moço, sou adivinho naum :D`, id)
